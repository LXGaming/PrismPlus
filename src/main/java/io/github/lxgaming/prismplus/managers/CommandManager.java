...
import io.github.lxgaming.prismplus.commands.AbstractCommand;
...
public final class CommandManager {
    ...
    public static CommandResult process(AbstractCommand abstractCommand, CommandSource commandSource, String message) {
        ...
        PrismPlus.getInstance().getLogger().debug("Processing {} for {}", command.get().getPrimaryAlias().orElse("Unknown"), commandSource.getName());
    ...
    public static boolean registerCommand(Class<? extends AbstractCommand> commandClass) {
        if (getCommandClasses().contains(commandClass)) {
            PrismPlus.getInstance().getLogger().warn("{} has already been registered", commandClass.getSimpleName());
            return false;
        }
        ...
        if (!command.isPresent()) {
            PrismPlus.getInstance().getLogger().error("{} failed to initialize", commandClass.getSimpleName());
            return false;
        }
        
        getCommands().add(command.get());
        Sponge.getCommandManager().register(PrismPlus.getInstance().getPluginContainer(), command.get(), command.get().getAliases().toArray(new String[0]));
        PrismPlus.getInstance().getLogger().debug("{} registered", commandClass.getSimpleName());
        return true;
    }
    ...
    public static boolean registerAlias(AbstractCommand command, String alias) {
        if (Toolbox.containsIgnoreCase(command.getAliases(), alias)) {
            PrismPlus.getInstance().getLogger().warn("{} has already been registered for {}", alias, command.getClass().getSimpleName());
            return false;
        }
        
        command.getAliases().add(alias);
        PrismPlus.getInstance().getLogger().debug("{} registered for {}", alias, command.getClass().getSimpleName());
        return true;
    }
    ...
    public static boolean registerCommand(AbstractCommand parentCommand, Class<? extends AbstractCommand> commandClass) {
        if (parentCommand.getClass() == commandClass) {
            PrismPlus.getInstance().getLogger().warn("{} attempted to register itself", parentCommand.getClass().getSimpleName());
            return false;
        }
        ...
        PrismPlus.getInstance().getLogger().debug("{} registered for {}", commandClass.getSimpleName(), parentCommand.getClass().getSimpleName());
        return true;
    }
